...
| `ssh_private_key_file` | no | managed per-VM Ed25519 key | Advanced override for the private key used by mutapod's pure-Go SSH client |
| `ssh_public_key_file` | no | managed matching public key | Advanced override for the public key passed to `az vm create --ssh-key-values` |
...
mutapod owns Azure SSH access by default. It creates a dedicated Ed25519 key under `~/.mutapod/keys/azure/`, passes that public key when provisioning a VM, and verifies real authentication before bootstrap. If the managed key is missing, corrupt, or no longer authorized on an existing VM, mutapod rotates it locally and reconciles its marked `authorized_keys` entry through Azure Run Command, then verifies SSH again. Unrelated authorized keys are preserved. The `ssh_private_key_file` and `ssh_public_key_file` settings remain available for advanced externally managed keys.

mutapod also writes its own OpenSSH host alias in `~/.ssh/config` for Mutagen. Non-interactive bootstrap and compose work use the same pure-Go SSH client as GCP. Interactive `mutapod ssh` delegates to `az ssh vm` so terminal handling stays normal.
...
If your Azure access is controlled by Privileged Identity Management (PIM), activate the required Azure resource role before any operation that reads, creates, starts, deallocates, or deletes the VM. Automatic recovery from a rejected SSH key also requires permission to run Azure VM Run Command. Once SSH is verified, normal sync/bootstrap/compose work uses SSH directly.
